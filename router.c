...
int forward_packet(packet_data_t *packet, int psize, routing_table_t *rt) {
    int size = rt->size, break_out=0, indice;
    for (int i = 0; i < size && break_out==0; i++){
        if (rt->tab[i].dest==packet->dst_id){
            break_out=1;
            indice=i;
        }
    }
    if (break_out==0){//Non trouvé
        return 0;
    }

     int sock_id;
     int port=rt->tab[indice].nexthop.port;
     char ip[16];
     strcpy(ip, rt->tab[indice].nexthop.ipv4);
...
void *hello(void *args) {
    ...
    while (1) {
        ...
        for (int i = 0; i < pargs->nt->size;    i++)
        {
            ...
            char ip[16];
            strcpy(ip, pargs->nt->tab[i].ipv4);
            struct sockaddr_in neigh_adr;
    ...
    while (1) {
        ...
        switch (buffer_in[0]) {
            ...
            case CTRL:
                ...
                for (int i = 0; i < pargs->nt->size; i++){
                    if (pargs->nt->tab[i].id == pctrl->src_id){
                        src = &(pargs->nt->tab[i]);
                        break;
                    }
                }
